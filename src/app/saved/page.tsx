...
export default function SavedPropertiesPage() {
  ...
  // 2. Fetch Saved Properties from Backend
  useEffect(() => {
    if (user?.id) {
  ...
  const fetchSavedProperties = async () => {
    setIsLoading(true);
    try {
      // Backend chi API: User ne save kelelya properties chi full detail ghene
      const response = await fetch(`http://localhost:8080/api/users/${user?.id}/favorites-details`);
      if (response.ok) {
  ...
  const removeFavorite = async (e: React.MouseEvent, propertyId: string) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/users/${user?.id}/favorites/${propertyId}`, {
        method: 'DELETE'
      });
      if (response.ok) {
        // UI madhun kadhun takne
              ...
              <div key={property.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden group hover:shadow-md transition-shadow">
                <Link href={`/properties/${property.id}`} className="block relative h-48">
                  {property.images && property.images.length > 0 ? (
                    ...
                    <div className="w-full h-full bg-gray-200 flex items-center justify-center"><Home className="h-10 w-10 text-gray-400" /></div>
                  )}
                  <button 
                    onClick={(e) => removeFavorite(e, property.id)}
                    className="absolute top-3 right-3 p-2 bg-white/90 backdrop-blur-sm rounded-full text-red-500 hover:bg-red-500 hover:text-white transition-all shadow-sm"
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-gray-50">
                    <span className="text-xl font-extrabold text-blue-600">₹{property.monthlyRent.toLocaleString('en-IN')}<span className="text-xs font-normal text-gray-500"> /mo</span></span>
